...
namespace DfE.FindInformationAcademiesTrusts.UnitTests.Services.ExportServices
{
    public class AcademiesExportServiceTests
    {
        ...
        public async Task IfTrustDetailsDontExist_ShouldThrow()
        {
            ...
            await result.Should().ThrowAsync<Exception>().WithMessage($"Trust summary is null for trust uid - {unknownTrustId}");
        }
        ...
        public async Task ExportAcademiesToSpreadsheet_ShouldCorrectlyExtractAcademyDataAsync()
        {
            ...
            _mockAcademyService.GetAcademiesInTrustDetailsAsync(trustUid).Returns([new("123456", "Academy 1", "Local Authority 1", "Type A", "Urban")]);
            _mockAcademyService.GetAcademiesInTrustOfstedAsync(trustUid).Returns([new("123456", "Academy 1", now, new OfstedRating(-1, null), new OfstedRating(1, now))]);
            _mockAcademyService.GetAcademiesInTrustPupilNumbersAsync(trustUid).Returns([new("123456", "Academy 1", "Primary", new AgeRange(5, 11), 500, 600)]);
            _mockAcademyService.GetAcademiesInTrustFreeSchoolMealsAsync(trustUid).Returns([new("123456", "Academy 1", 20, 25, 15)]);
        ...
        {
            _mockAcademyService.GetAcademiesInTrustDetailsAsync(trustUid).Returns([new("123456", null, null, null, null)]);

            var now = DateTime.Now;
            _mockAcademyService.GetAcademiesInTrustOfstedAsync(trustUid).Returns([new("123456", null, now, new OfstedRating(-1, null), new OfstedRating(-1, null))]);
            _mockAcademyService.GetAcademiesInTrustPupilNumbersAsync(trustUid).Returns([new("123456", null, null, new AgeRange(5, 11), null, null)]);
            _mockAcademyService.GetAcademiesInTrustFreeSchoolMealsAsync(trustUid).Returns([new("123456", null, null, 25, 35)]);
            ...
            worksheet.CellValue(4, (int)AcademyColumns.LaPupilsEligibleFreeSchoolMeals).Should().Be("25%");
            worksheet.CellValue(4, (int)AcademyColumns.NationalPupilsEligibleFreeSchoolMeals).Should().Be("35%");
        }
    }
