...
public class ContactAreaModelTests
{
    ...
    public ContactAreaModelTests()
    {
        ...
        _sut = new ContactAreaModel(_mockDataSourceService.Object, _mockTrustService.Object,
                new MockLogger<ContactAreaModel>().Object, model => model.Page == "./InDfE")
            { Uid = TestUid };
    }
    ...
    public async Task OnGetAsync_sets_correct_SubNavigationLinks()
    {
        ...
        _sut.SubNavigationLinks.Should().BeEquivalentTo([
            new TrustSubNavigationLinkModel("In DfE", "./InDfE", TestUid, "Contacts") { LinkIsActive = true },
            new TrustSubNavigationLinkModel("In the trust", "./InTrust", TestUid, "Contacts")
        ]);
    }
