...
export class Autocomplete {
  suggest = async (query, populateResults, inputId) => {
    ...
    // Avoids the uid turning up in the url
      const searchInput = document.getElementById(`${inputId}-selected-identifier`)
  ...
  loadTrustSearch = async (inputId, defaultValue, placeholderText) => {
    ...
    accessibleAutocomplete({
      ...
      onConfirm: (selected) => {
        if (selected) {
            document.getElementById(`${inputId}-selected-identifier`).value = (selected.identifier)
            document.getElementById(`${inputId}-selected-resultType`).value = (selected.resultType)
        }
      }
    })
