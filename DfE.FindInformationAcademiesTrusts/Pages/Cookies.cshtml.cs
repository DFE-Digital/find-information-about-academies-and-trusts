...
public class CookiesModel : PageModel
{
    ...
    {
        _httpContextAccessor.HttpContext?.Response.Cookies.Delete("ai_session", new CookieOptions { Path = "/" });
        _httpContextAccessor.HttpContext?.Response.Cookies.Delete("ai_user", new CookieOptions { Path = "/" });
        _httpContextAccessor.HttpContext?.Session.SetInt32("CookieStatus",
