        ...
        public byte[] ExportAcademiesToSpreadsheetUsingProvider(Data.Trust trust, Trust.TrustSummaryServiceModel? trustSummary)
        {
            ...
            var dataExtractor = new Func<Data.Academy, string[]>((academy) =>
             [
                 ...
                 IsOfstedRatingBeforeOrAfterJoining(academy.PreviousOfstedRating?.OfstedRatingScore ?? OfstedRatingScore.None, academy.DateAcademyJoinedTrust, academy.PreviousOfstedRating?.InspectionDate).ToString(),
                 academy.PreviousOfstedRating?.InspectionDate?.ToString(StringFormatConstants.ViewDate) ?? string.Empty,
                 academy.CurrentOfstedRating?.OfstedRatingScore.ToString() ?? string.Empty,
                 IsOfstedRatingBeforeOrAfterJoining(academy.CurrentOfstedRating?.OfstedRatingScore ?? OfstedRatingScore.None, academy.DateAcademyJoinedTrust, academy.CurrentOfstedRating?.InspectionDate).ToString(),
                 academy.CurrentOfstedRating?.InspectionDate?.ToString(StringFormatConstants.ViewDate) ?? string.Empty,
